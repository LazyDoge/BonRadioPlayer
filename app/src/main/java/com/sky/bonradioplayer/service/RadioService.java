...
import com.sky.bonradioplayer.ui.MainActivity;
...
public class RadioService extends Service {
    ...
    public static String title = "Сейчас в эфире: \n скоро будет добавлено";
...
//        title = "Сейчас в эфире: \n скоро будет добавлено";
        ...
        DataSource.Factory httpDataSourceFactory = new OkHttpDataSourceFactory(new OkHttpClient(), Util.getUserAgent(this, getString(R.string.app_name_en)), null);
...
//            Log.d("onAudioFocusChange", String.valueOf(focusChange));
            ...
            switch (focusChange) {
                case AudioManager.AUDIOFOCUS_GAIN:
                    mediaSessionCallback.onPause(); // Не очень красиво
                    break;
                case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK:
                    mediaSessionCallback.onPause();
                    break;
                case AudioManager.ERROR:
    ...
    private ExoPlayer.EventListener exoPlayerListener = new ExoPlayer.EventListener() {
        ...
        public void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
            Log.d("onPlayerStateChanged", String.valueOf(playbackState) + String.valueOf(playWhenReady));
...
//            notificationManager.cancel(NOTIFICATION_ID);
            ...
            exoPlayer = null;
            exoPlayer = ExoPlayerFactory.newSimpleInstance(new DefaultRenderersFactory(getApplicationContext()), new DefaultTrackSelector(), new DefaultLoadControl());
            exoPlayer.addListener(exoPlayerListener);
            MusicRepository.Track track = musicRepository.getCurrent();
            ExtractorMediaSource mediaSource = new ExtractorMediaSource(track.getUri(), dataSourceFactory, extractorsFactory, null, null);
            exoPlayer.prepare(mediaSource);
            registerReceiver(becomingNoisyReceiver, new IntentFilter(AudioManager.ACTION_AUDIO_BECOMING_NOISY));
...
//            Toast.makeText(RadioService.this, "Произошла ошибка! Перезапустите приложение Радио БОН", Toast.LENGTH_LONG).show();
...
//            onCreate();
    ...
    private void refreshNotificationAndForegroundStatus(int playbackState) {
        switch (playbackState) {
            ...
            case PlaybackStateCompat.STATE_PAUSED: {
                NotificationManagerCompat.from(RadioService.this).notify(NOTIFICATION_ID, getNotification(playbackState));
    ...
    private Notification getNotification(int playbackState) {
        ...
        if (playbackState == PlaybackStateCompat.STATE_PLAYING) {
            builder.addAction(new NotificationCompat.Action(R.drawable.exo_controls_pause, getString(R.string.pause), MediaButtonReceiver.buildMediaButtonPendingIntent(this, PlaybackStateCompat.ACTION_PLAY_PAUSE)));
        }
        else {
            builder.addAction(new NotificationCompat.Action(R.drawable.exo_controls_play, getString(R.string.play), MediaButtonReceiver.buildMediaButtonPendingIntent(this, PlaybackStateCompat.ACTION_PLAY_PAUSE)));
        }

//        builder.addAction(new NotificationCompat.Action(android.R.drawable.menuitem_background, getString(R.string.next), MediaButtonReceiver.buildMediaButtonPendingIntent(this, PlaybackStateCompat.ACTION_SKIP_TO_NEXT)));
        builder.addAction(new NotificationCompat.Action(android.R.drawable.ic_lock_power_off, getString(R.string.previous), MediaButtonReceiver.buildMediaButtonPendingIntent(this, PlaybackStateCompat.ACTION_STOP)));
        ...
        builder.setColor(ContextCompat.getColor(this, R.color.darkColor)); // The whole background (in MediaStyle), not just icon background
